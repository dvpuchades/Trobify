...
class PlacesService {
  final key = 'put your own api key';

  Future<List<PlaceSearch>> getAutocomplete(String search) async {
    var url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$key&language=es');
  ...
  Future<Place> getPlace(String placeId) async {
    var url = Uri.parse(
        'https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&key=$key&language=es');
    var response = await http.get(url);
    var json = convert.jsonDecode(response.body);
